...
def read_data(protein_name):
    ...
    for (ntmpi, ntomp) in arg_combs:
        dirname = "run_{}_{}_{}".format(protein_name, ntmpi, ntomp)
        x.append(ntmpi)
        y_tmp_raw = []
        for i in range(5):
...
def gen_txt():
    logfile = open("FFComparision_scaling.txt", 'w')
    res = [[0]*5 for _ in range(4)]
    res[0][0]=2; res[1][0]=3; res[2][0]=4; res[3][0]=8;
    for (i, fld) in enumerate(fields):
        x, y_raw, y_avg, y_err = read_data(fld)
        for j in range(4):
            res[j][i+1] = y_avg[j]
    for fld in fields:
        print(fld + "    ", end="")
    ...
    for line in res:
        for item in line:
            logfile.write(str(round(item, 4)) + "\t")
        logfile.write("\n")
    logfile.close()
    # print(res)
